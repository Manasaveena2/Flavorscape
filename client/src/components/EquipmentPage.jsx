...
const EquipmentPage = () => {
  ...
  const [selectedCuisine, setSelectedCuisine] = useState('frying-pan'); // Default cuisine is set to 'Indian'
  
  const apiKey = '2cf363e464msh40119dd71827212p1759f2jsn9f2d6345d821'; // Replace with your API key
  const apiUrl = `https://spoonacular-recipe-food-nutrition-v1.p.rapidapi.com/recipes/complexSearch?equipment=frying-pan&addRecipeNutrition=true&number=100`;
  ...
  const handleSearch = () => {
    ...
    if (selectedCuisine.trim() !== '') queryParams.push(`equipment=${encodeURIComponent(selectedCuisine)}`);
